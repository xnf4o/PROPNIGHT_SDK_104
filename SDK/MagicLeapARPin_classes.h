...
public:
	...
	unsigned char                                      UnknownData_35AN[0x2];                                     // 0x020E(0x0002) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	unsigned char                                      UnknownData_699Y[0xF0];                                    // 0x02C0(0x00F0) MISSED OFFSET (PADDING)
...
public:
	...
	void UnBindToOnMagicLeapContentBindingFoundDelegate(const struct FScriptDelegate& Delegate);
	void UnBindToOnMagicLeapARPinUpdatedDelegate(const struct FScriptDelegate& Delegate);
	MagicLeapARPin_EMagicLeapPassableWorldError SetGlobalQueryFilter(const struct FMagicLeapARPinQuery& InGlobalFilter);
	void SetContentBindingSaveGameUserIndex(int UserIndex);
	MagicLeapARPin_EMagicLeapPassableWorldError QueryARPins(const struct FMagicLeapARPinQuery& Query, TArray<struct FGuid>* Pins);
	bool ParseStringToARPinId(const struct FString& PinIdString, struct FGuid* ARPinId);
	bool IsTrackerValid();
	MagicLeapARPin_EMagicLeapPassableWorldError GetNumAvailableARPins(int* Count);
	MagicLeapARPin_EMagicLeapPassableWorldError GetGlobalQueryFilter(struct FMagicLeapARPinQuery* CurrentGlobalFilter);
	int GetContentBindingSaveGameUserIndex();
	MagicLeapARPin_EMagicLeapPassableWorldError GetClosestARPin(const struct FVector& SearchPoint, struct FGuid* PinId);
	MagicLeapARPin_EMagicLeapPassableWorldError GetAvailableARPins(int NumRequested, TArray<struct FGuid>* Pins);
	struct FString GetARPinStateToString(const struct FMagicLeapARPinState& State);
	...
	void BindToOnMagicLeapContentBindingFoundDelegate(const struct FScriptDelegate& Delegate);
	void BindToOnMagicLeapARPinUpdatedDelegate(const struct FScriptDelegate& Delegate);
	struct FString ARPinIdToString(const struct FGuid& ARPinId);
};
...
public:
	...
	unsigned char                                      UnknownData_T2FM[0x7];                                     // 0x0231(0x0007) MISSED OFFSET (PADDING)
...
public:
	...
	unsigned char                                      UnknownData_5WMC[0x7];                                     // 0x0221(0x0007) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	TMap<struct FGuid, class AMagicLeapARPinInfoActorBase*> AllInfoActors;                                             // 0x0228(0x0050) (NativeAccessSpecifierPrivate)
	unsigned char                                      UnknownData_TXBX[0x8];                                     // 0x0278(0x0008) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
...
public:
	...
	unsigned char                                      UnknownData_IO4E[0x8];                                     // 0x0038(0x0008) MISSED OFFSET (FIX SPACE BETWEEN PREVIOUS PROPERTY)
	...
	unsigned char                                      UnknownData_NP0S[0xF];                                     // 0x00A1(0x000F) MISSED OFFSET (PADDING)
